...
const PROJECT_CONFIG = {
  ...
  /** Build previo (Angular u otro stack) antes de publicar */
  build: {
    nodeVersion: '22',
    cacheDependencyPath: 'frontend/package-lock.json',
    steps: [
      { name: 'Install frontend', workingDirectory: 'frontend', run: 'npm ci' },
      { name: 'Build GitHub Pages', workingDirectory: 'frontend', run: 'npm run build:github' },
      {
        name: 'SPA fallback (404 → index.html)',
  ...
  deployVerifyOptional: ['404.html', '.nojekyll'],

  /** Archivos para CI smoke test (relativos a vitrina). Vacío = solo index.html */
  ciSmokeFiles: ['assets/img/logo.png', 'assets/img/logo_favicon.png'],
...
function detectStack(root) {
  ...
  if (exists(path.join(root, 'frontend', 'package.json'))) {
    ...
    if (pkg.dependencies?.['@angular/core'] || pkg.devDependencies?.['@angular/cli']) {
      notes.push('Angular detectado — build con npm run build:github antes de publicar.')
    }
  }
...
function buildWorkflowBuildSteps() {
  const cfg = PROJECT_CONFIG.build
  if (!cfg?.steps?.length) return ''
  const nodeSetup = cfg.nodeVersion
    ? `      - name: Setup Node.js
        uses: actions/setup-node@v4
        with:
          node-version: '${cfg.nodeVersion}'
          cache: 'npm'
          cache-dependency-path: ${cfg.cacheDependencyPath || 'package-lock.json'}
`
    : ''
  const steps = cfg.steps
    .map((s) => {
      const wd = s.workingDirectory ? `\n        working-directory: ${s.workingDirectory}` : ''
      return `      - name: ${s.name}${wd}
        run: ${s.run}`
    })
    .join('\n\n')
  return nodeSetup + steps + '\n\n'
}

function buildCiBuildSteps() {
  return buildWorkflowBuildSteps()
}
...
${buildWorkflowBuildSteps()}      - name: Verificar vitrina (index.html en raíz del artefacto)
        run: |
          ${verifyRun}
...
${buildCiBuildSteps()}      - name: Verificar estructura mínima
        run: |
          ${verifyRun}
